...
function PLUGIN:PostInstall(ctx)
    ...
    --[[
    -- Archive already contains bin/screenshotbot, just verify it works
    local testResult = os.execute(path .. "/bin/screenshotbot --version > /dev/null 2>&1")
    if testResult ~= 0 then
        error("screenshotbot installation appears to be broken")
    end
    --]]
